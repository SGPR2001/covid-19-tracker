...
import "./WorstEffectedState.css";

export default function WorstEffectedState({
  WorstEffectedStates,
  statescoviddata,
  selectedDays,
}) {
  ...
  useEffect(() => {
    const stateData = statescoviddata.find((state) =>
      WorstEffectedStates.includes(state.name)
    );
    if (stateData) {
      setStateName(stateData.name);
      // const dates = Object.keys(stateData[plotType]).slice(-selectedDays);
      // const plotData = Object.values(stateData[plotType]).slice(-selectedDays);
      let dates = [];
      let plotData = [];
      ...
      if (plotType === "TotalCases") {
        ...
        dates = Object.keys(stateData.TotalRecovered).slice(-selectedDays);
        const totalRecovered = Object.values(stateData.TotalRecovered).slice(
          -selectedDays
        );
        ...
        plotData = Object.values(stateData[plotType]).slice(-selectedDays);
      }
      if (chartRef.current) {
  ...
  }, [WorstEffectedStates, statescoviddata,plotType]);

  return (
    <div className="worststates">
      <h2>Worst Effected State-{stateName}</h2>
